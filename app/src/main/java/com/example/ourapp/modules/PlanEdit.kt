...
class PlanEdit : AppCompatActivity() {
    private var list: MutableList<PlanAddPhotoItem> = mutableListOf()
    private val UPDATEPLAN: Int = 0
    private lateinit var dateView: TextView
    private var handler: Handler = Handler {
        when(it.what) {
            UPDATEPLAN -> {
                dateView.text = plan.date
                editPlan.setText(plan.content)
                // 将光标移动到末尾
                editPlan.setSelection(plan.content.length)
                val images = plan.imgs.split(",")
                var bitmap: Bitmap?
                for (image in images) {
                    bitmap = Utils.base64ToBitmap(image)
                    if (bitmap != null) {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.module_plan_edit)
        // textView
    ...
    private fun savePlan () {
        ...
        values.put("imgs", imgs.joinToString())
        showPop()
        loadingPlan.visibility = View.VISIBLE
        LitePal.updateAsync(PlanDb::class.java, values, planId).listen {
